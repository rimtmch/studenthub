...
import { Sun, Moon, Coffee, Edit2, BookOpen, X, Save, Loader2, Calendar, Utensils, HeartPulse, Activity, Store, ClipboardCheck } from 'lucide-react';
...
const Home: React.FC<HomeProps> = ({ user }) => {
  ...
  const fetchHealthDays = async (day: number, month: number) => {
      try {
          ...
          console.error("fetchHealthDays failed gracefully:", e);
      }
  };
  ...
  const fetchMenuForDay = async (day: string, isInitialLoad = false) => {
      ...
      try {
          ...
          console.error("fetchMenuForDay failed gracefully:", e);
      }
      ...
      <div className="glass rounded-[3rem] p-8 sm:p-10 text-white shadow-[0_20px_50px_rgba(0,0,0,0.15)] relative overflow-hidden group border border-white/20">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-8">
            ...
            <div className="flex flex-wrap gap-4 w-full md:w-auto">
                ...
                <Link to="/book-renting" className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-white/20 hover:bg-white/30 text-slate-900 dark:text-white px-8 py-4 rounded-2xl font-bold text-sm border border-white/20 backdrop-blur-md shadow-lg transition transform hover:-translate-y-1 active:scale-95">
                    <Store className="w-4 h-4" /> Rent Books
                </Link>
            </div>
        </div>
        ...
        <div className="md:col-span-2 space-y-6">
          <div className="glass rounded-[2.5rem] p-8 border border-white/20 shadow-xl">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <Link to="/attendance" className="group bg-slate-50/50 dark:bg-white/5 p-6 rounded-[2rem] border border-white/20 flex flex-col items-center justify-center gap-4 hover:bg-white/40 dark:hover:bg-white/10 transition duration-300 shadow-sm hover:shadow-lg">
                <div className="p-4 bg-rose-500/10 dark:bg-rose-500/20 rounded-2xl text-rose-600 dark:text-rose-300 group-hover:scale-110 transition duration-500 shadow-inner">
                  <ClipboardCheck className="w-7 h-7" />
                </div>
                <span className="text-sm font-semibold text-slate-700 dark:text-slate-300 group-hover:text-rose-600 dark:group-hover:text-white transition">Attendance</span>
              </Link>

              <Link to="/book-renting" className="group bg-slate-50/50 dark:bg-white/5 p-6 rounded-[2rem] border border-white/20 flex flex-col items-center justify-center gap-4 hover:bg-white/40 dark:hover:bg-white/10 transition duration-300 shadow-sm hover:shadow-lg">
                <div className="p-4 bg-emerald-500/10 dark:bg-emerald-500/20 rounded-2xl text-emerald-600 dark:text-emerald-300 group-hover:scale-110 transition duration-500 shadow-inner">
                  <Store className="w-7 h-7" />
                </div>
                <span className="text-sm font-semibold text-slate-700 dark:text-slate-300 group-hover:text-emerald-600 dark:group-hover:text-white transition">Book Rent</span>
              </Link>

              <Link to="/social" className="group bg-slate-50/50 dark:bg-white/5 p-6 rounded-[2rem] border border-white/20 flex flex-col items-center justify-center gap-4 hover:bg-white/40 dark:hover:bg-white/10 transition duration-300 shadow-sm hover:shadow-lg">
                <div className="p-4 bg-amber-500/10 dark:bg-amber-500/20 rounded-2xl text-amber-600 dark:text-amber-300 group-hover:scale-110 transition duration-500 shadow-inner">
                  <Coffee className="w-7 h-7" />
                </div>
                <span className="text-sm font-semibold text-slate-700 dark:text-slate-300 group-hover:text-amber-600 dark:group-hover:text-white transition">Chill Corner</span>
              </Link>
            </div>
          </div>
